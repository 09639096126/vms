...
def context_data(request):
    ...
    context = {
        ...
        'page_name': '',
        'page_title': '',
        'system_name': 'ICT Visitor Managament System',
        'topbar': True,
        'footer': True,
    }
...
def home(request):
    ...
    context['dapartments'] = models.Departments.objects.filter(
        delete_flag=0, status=1).all().count()
...
def departments(request):
    ...
    context['departments'] = models.Departments.objects.filter(
        delete_flag=0).all()
...
def save_department(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if post['id'] == '':
                messages.success(
                    request, "Department has been saved successfully.")
            else:
                messages.success(
                    request, "Department has been updated successfully.")
            resp['status'] = 'success'
        else:
            for field in form:
                for error in field.errors:
...
def users(request):
    ...
    context['users'] = User.objects.exclude(
        pk=request.user.pk).filter(is_superuser=False).all()
...
def save_visitor(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if post['id'] == '':
                messages.success(
                    request, "Visitor has been saved successfully.")
            else:
                messages.success(
                    request, "Visitor has been updated successfully.")
            resp['status'] = 'success'
        else:
            for field in form:
                for error in field.errors:
